...
export async function POST(request: NextRequest) {
  try {
    ...
    // Parse request body
    const { username, tweetCount = 200, forceRefresh = false, useApi = true } = await request.json();
    ...
    const { data: job, error: jobError } = await supabase
      ...
      .insert({
        ...
        parameters: {
          ...
          forceRefresh,
          useApi,
          platform_required: 'twitter_harvester'
        },
        progress: 0
      })
      .select()
    ...
    return NextResponse.json({ 
      jobId: job.id,
      message: 'Twitter harvest job created',
      status: 'pending'
    });
  } catch (error) {
    console.error('Twitter harvest error:', error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to process Twitter harvest' },
      { status: 500 }
    );
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Twitter job status error:', error);
    return NextResponse.json(
