...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!handle && !profileId) {
      return NextResponse.json({ error: 'Instagram handle or profile ID is required' }, { status: 400 });
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to get Instagram profile' },
      { status: 500 }
    );
  }
}