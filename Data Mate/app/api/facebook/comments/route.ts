  ...
  try {
    ...
    if (topLevelOnly && comments.length > 0) {
      ...
      for (const comment of comments) {
        ...
        if (includeMedia) {
          comment.media = comment.content_media || [];
        }
      }
    } else if (includeMedia) {
      // If not fetching replies but including media, format the comments
      for (const comment of comments) {
        comment.media = comment.content_media || [];
      }
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to get Facebook comments' },
      { status: 500 }
    );
  }
}