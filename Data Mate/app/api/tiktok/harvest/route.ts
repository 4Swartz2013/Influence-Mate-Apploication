...
export async function POST(request: NextRequest) {
  try {
    ...
    // Parse request body
    const { handle, videoCount = 50, forceRefresh = false, useApi = true } = await request.json();
    ...
    const { data: job, error: jobError } = await supabase
      ...
      .insert({
        ...
        parameters: {
          ...
          forceRefresh,
          useApi,
          platform_required: 'tiktok_harvester'
        },
        progress: 0
      })
      .select()
    ...
    return NextResponse.json({ 
      jobId: job.id,
      message: 'TikTok harvest job created',
      status: 'pending'
    });
  } catch (error) {
    console.error('TikTok harvest error:', error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to process TikTok harvest' },
      { status: 500 }
    );
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('TikTok job status error:', error);
    return NextResponse.json(
