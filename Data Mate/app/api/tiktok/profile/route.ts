...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!handle && !profileId) {
      return NextResponse.json({ error: 'TikTok handle or profile ID is required' }, { status: 400 });
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to get TikTok profile' },
      { status: 500 }
    );
  }
}